...
public class SMSAuthenticator implements Authenticator {
	...
	public void authenticate(AuthenticationFlowContext context) {
		...
		logger.debugv("countryCode : ", countryCode);
		...
		if (phoneNumber != null) {
			...
			if (sendVerify.sendSMS(phoneNumber , countryCode)) {
	...
	public void action(AuthenticationFlowContext context) {
		...
		if (sendVerify.verifySMS(phoneNumber, enteredCode , countryCode )) {
	...
	private String getCountryCode(UserModel user) {
		List<String> code = user.getAttribute(SMSAuthContstants.COUNTRY_CODE);
		
		logger.infov("Country code is :" + code.get(0));
