...
import jp.openstandia.keycloak.authenticator.SMSAuthenticator;
...
public class SMSSendVerify {
	...
	SMSAuthenticator smsAuth=new SMSAuthenticator();
	...
	public boolean sendSMS(String telNum , String countryCode) {
		
		

		SMSParams data = new SMSParams();
		data.setAttribute("phone_number", telNum);
		//data.setAttribute("country_code", "91"); // for India Country code only
		data.setAttribute("country_code", countryCode);  
		data.setAttribute("via", "sms"); // SMS
		data.setAttribute("code_length", codeLen);
		//logger.infov("Country code is :" +SMSAuthContstants.COUNTRY_CODE);

		return request(METHOD_POST, PHONE_VERIFICATION_API_PATH + "start", data);
	}

	public boolean verifySMS(String telNum, String code, String countryCode ) {

		SMSParams data = new SMSParams();
		data.setAttribute("phone_number", telNum);
		data.setAttribute("country_code", countryCode ); // for all Country code
		//data.setAttribute("country_code", "91");  // for India Country code only
